...
export function StoryWorld({ quality, onFirstSceneReady }: { quality: "high" | "medium"; onFirstSceneReady?: () => void }) {
  ...
  return (
    <>
      ...
      <SceneAsset key={sceneDefinitions[from].id} definition={sceneDefinitions[from]} sceneIndex={from} onReady={from === 0 ? onFirstSceneReady : undefined} />
      {to !== from && <SceneAsset key={sceneDefinitions[to].id} definition={sceneDefinitions[to]} sceneIndex={to} />}
    </>
  );
}
