...
export function PerformanceGovernor({ quality }: { quality: "high" | "medium" }) {
  ...
  useFrame((_, delta) => {
    ...
    if (fps < 54 && current > limits.minDpr + 0.03) {
      slowWindows.current += 1;
      stableWindows.current = 0;
      if (slowWindows.current >= 2) {
        renderer.setPixelRatio(Math.max(limits.minDpr, current - 0.1));
        renderer.setSize(size.width, size.height, false);
        slowWindows.current = 0;
      }
    } else if (fps > 59 && current < limits.maxDpr - 0.03 && budgetWarnings.length === 0) {
      stableWindows.current += 1;
      slowWindows.current = 0;
      if (stableWindows.current >= 4) {
        renderer.setPixelRatio(Math.min(limits.maxDpr, current + 0.06));
        renderer.setSize(size.width, size.height, false);
        stableWindows.current = 0;
      }
    } else {
