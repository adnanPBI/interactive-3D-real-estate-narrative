...
for (const file of required) {
  if (!fs.existsSync(file)) throw new Error(`R4 missing required file: ${path.relative(root, file)}`);
}

const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
if (manifest.stage !== '6-r4-visual-fidelity') throw new Error(`Unexpected R4 manifest stage: ${manifest.stage}`);
if (!Array.isArray(manifest.assets) || manifest.assets.length !== 6) throw new Error('R4 manifest must contain exactly six assets.');
...
for (const name of names) {
  ...
  if (!item) throw new Error(`R4 manifest missing ${name}.glb`);
...
if (!scenes.includes('requestedAssetSet === "stage3" ? "stage3" : "r4"')) throw new Error('R4 is not the default runtime asset set.');
if (!scenes.includes('fallbackBase = assetSet === "r4" ? "/fallback/r4"')) throw new Error('R4 fallback routing missing.');
for (const x of ['[11.0, 6.7, 15.8]','[8.2, 4.55, 10.8]','[6.2, 6.25, 14.8]','[3.2, 4.75, 11.7]','[1.0, 4.4, 11.5]','[-1.0, 7.1, 17.8]']) {
  if (!scenes.includes(x)) throw new Error(`R4 camera anchor missing: ${x}`);
}

const sceneAsset = fs.readFileSync(sceneAssetPath, 'utf8');
for (const token of ['material.name === "Facade"','material.name === "WarmGlow"','material.name === "CoolGlow"','shouldWriteDepth = fade > 0.965']) {
  if (!sceneAsset.includes(token)) throw new Error(`R4 material/depth hardening missing: ${token}`);
}
