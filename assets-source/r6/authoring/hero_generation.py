...
def build(ctx, lod: str):
    ...
    for i,x in enumerate(np.linspace(-.2,5.8,bay_count)):
        ...
        for dz in (-.34,0,.34):
            b.cyl(.085,.44,(float(x),1.30,.45+dz),"Bronze",sections=18 if high else 12)
        for pole in (-.32,.32):
    ...
    for i,x in enumerate(np.linspace(-5.8,-1.8,5 if high else 3)):
        ...
        if lod in ("lod0","lod1"):
            for yy in (.30,.48,.66,.84):
                b.box((.62,.025,.035),(float(x),yy,2.455),"Aluminum")
            b.box((1.18,.08,2.28),(float(x),.04,1.35),"Concrete")

    if lod in ("lod0","lod1"):
        r5.insulator_bank(b,(2.8,0,-2.65),11 if high else 7,5 if high else 3,.78)
        r5.gantry_detail(b,-.8,6.4,-1.30,-.40,2.55,7 if high else 5,high)
        r5.cable_tray(b,(-.4,0,-1.55),(6.0,0,-1.55),2.15,5 if high else 3,.78)
        for x in (-5.65,-4.65,-3.65,-2.65):
